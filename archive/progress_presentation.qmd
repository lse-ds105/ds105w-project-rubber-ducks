...
```{mermaid}
flowchart LR
    A{open-meteo} --> L((open-meteo API<br>Requests))
    L --> B[London<br>Dataframe]
    L --> C[Bangkok<br>Dataframe]
    L --> D[ ... ]
    L --> E[Hong Kong<br>Dataframe]
    G{top 20 cities website} -->|scraping| H(python list of<br>20 cities)
    I{OpenStreetMaps} --> J((OSM API<br>request))
    H --> J
    J --> K(python list of dictionaries<br>for coordinates of all cities)
    K --> L
    ...
    S -->|scraping| T[dataframe of appearance<br>% for each query]
    ...
    V -->|Data manipulation| R(More Complex and Interactive<br>Visualisations)
```
